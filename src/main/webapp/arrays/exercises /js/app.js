...
//
// Napisz funkcję ```distFromAverage```, która ma przyjmować
// tylko jeden argument &ndash; tablicę. Funkcja ta ma zwracać też tablicę.
...
function distFromAverage(tab) {
    let sum = tab.reduce(function (total, item) {
        return total + item;
...
// Następnie przy pomocy odpowiedniej metody tablicowej przeiteruj (przejdź) po każdym elemencie
// i wypisz każdy z nich w konsoli.
//
//
//     ## Zadanie 5
//
// Napisz funkcję ```multiply(array)```. Funkcja ma przyjmować tylko jeden argument - tablicę.
// Następnie funkcja ```multiply``` ma pomnożyć wszystkie liczby znajdujące się w tablicy przy pomocy
// odpowiedniej metody tablicowej i zwrócić wynik.
//

function multiply(array) {
    let result = array.reduce(function (preElement, element) {
...
// ## Zadanie 6
//
// Napisz funkcję ```getEvenAvarage```, która ma przyjmować tylko jeden argument - tablicę.
...
function getEvenAverage(array) {
    let evenElements = array.filter(function (element) {
        if (element % 2 === 0) {
            return element;
        }
    });
    if (evenElements.length !== 0) {
        let sum = evenElements.reduce(function (preElement, element) {
            return preElement + element;
        });
...
// ## Zadanie 7
//
// Napisz funkcję ```sortArray```, która ma przyjmować tylko jeden argument - tablicę zawierającą
// liczby całkowite. Funkcja ta ma zwracać posortowaną rosnąco tablicę. Możesz skorzystać z metod tablicowych.
//
//     ```JavaScript
